...
uchar error_id;
@near bool skip_error;

@near clock_t error_time;

#define CP_SIZE 5
@near static uchar currents[CP_SIZE];
@near static uint powers[CP_SIZE];
@near static uchar cp_index;
@near static uint cp_count;
@near static union {
    uint avg;
    ulong accu;
} current, power;
