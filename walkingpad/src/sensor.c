...
@near static ulong array[QSIZE];
@near static ulong array2[QSIZE];
@near static uchar filter_cnt, filter_cnt2;
@near static uchar all_display_on_cnt_old;
@near uchar flag_Gsensor_disconnected;

@near ulong tension, tension2;
@near ulong tension_orig, tension2_orig;
@near static ulong tension_ini, tension2_ini;
@near long tension_bias, tension2_bias;

@near static long tension_old, tension2_old;

@near static uchar array_idx;

@near static uchar drift_cnt;
@near uchar no_current_cnt;
//@near uchar fold_cnt;

@near static uchar disconnect_cnt1, disconnect_cnt2;
//@near static uchar disconnect_sec1, disconnect_sec2;

//@near long tension_abs, tension2_abs;
@near static ulong drift, drift2;
@near static ulong drift_abs, drift2_abs;

@near static uchar sensor_release_cnt;
@near static uchar sensor_press_cnt;
@near bool flag_sensor_may_reverted;
@near extern user_state_t userstate;
@near ebool flag_may_stuck;
...
static ulong get_median(ulong data_array[])
{
    ...
    memcpy(data, data_array, sizeof(data));
...
#else
static ulong get_mean(ulong data[])
{
