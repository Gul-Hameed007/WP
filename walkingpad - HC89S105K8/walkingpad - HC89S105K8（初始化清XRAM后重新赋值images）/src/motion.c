...
static uchar ui_state;
 ...
 static uchar user_machine_state_cnt;  //count for user and machine state check, in second
 static uchar user_machine_state_cnt1; //count for user and machine state check, in second
 static uchar show_for_a_while;

 uint display_cnt;
 display_seg_t display_seg;
 bool start_no_tick;
 ulong user_total_distance;

 bool show_factory_mode;

 ebool flag_sensor_may_reverted;
 ...
 *--------------------------------------------------------------------------*/
 ...
 static uchar const code lib_rc_start[LED_DOWN_SIZE] =
    {
...
void UserConsumerDisplay(void)
{
    ...
    uint digit_int;
    extern const uchar code FW_VERSION[];
