...
@near bool flag_mode_changed;
@near uchar commu_wifi_flag;
@near static uchar mcu2wifi_txd[BUFF_LEN], mcu2wifi_rxd[BUFF_LEN], ack_str[BUFF_LEN] = {0};
@near static uchar commu_mcu2wifi_cnt;
@near static uchar command_mcu2wifi_size;
@near static uchar cnt_mcu2wifi_tra2, cnt_mcu2wifi_rec2;
@near static uchar transmit_delay;
@near volatile static bool recvorder;
@near uint button_id;
@near ulong server_time;
...
@near net_state_t net_state;
@near struct_mp_t store_mp;
@near struct_point_t store_point;

static const uchar *const MODEL = "model";
static const uchar *const MODEL_NAME = "ksmb.walkingpad.v1";
const uchar *const FW_VERSION = "0034";
static const uchar *const OK = "ok";
...
void commu_wifi(void)
{
    ...
    if (commu_mcu2wifi_state == COMMU_STATE_MCU2WIFI_TRANSMIT)
    {
        ...
        else if (mcu2wifi_cmd == MCU_WIFI_NO_COMMAND)
        {
            ...
            else if ((props_time_second != user_time_second
...
@far @interrupt void txd_isr(void)
{
    if (cnt_mcu2wifi_tra2 < command_mcu2wifi_size)
    {
        UART2_DR = mcu2wifi_txd[cnt_mcu2wifi_tra2++];
    }
    else
    {
        cnt_mcu2wifi_tra2 = 0;
...
@far @interrupt void rxd_isr(void)
{
    mcu2wifi_rxd[cnt_mcu2wifi_rec2] = UART2_DR;
