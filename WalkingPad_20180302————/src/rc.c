...
volatile static int rcTimer = 0; // clear at head or overflow, plus at every timer
...
@far @interrupt void rc_isr(void)
{
	if (rcState == 0)
...
@far @interrupt void TIM2_OV_isr(void)
{
	TIM2_SR1 &= 0xFE; //clear Flag
